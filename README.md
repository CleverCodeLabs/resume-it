...
run the following commands into the root directory:

```
docker run --name mongodb -d --rm -p 27017:27017 \
    -v $(pwd)/data:/data/db \
...
or run `docker-compose up -d` into the root directory.
...
run the following command from `apps/api` directory:
